...
Any program may read this JSON: Ark, Ansible, a small Go binary, nginx or Caddy generators, or a VPS-side `muxd`. The Rust crate **`ark-protocol`** (`crates/ark-protocol`, import **`ark_protocol`**) exposes `manifest_json_for_deploy`, and the npm package **`@tschk/ark-protocol`** (`packages/ark-protocol`) exposes `manifestJsonForDeploy`, so control planes or CLIs in Rust, Bun, or Node can emit the same JSON string as the Worker binding without reimplementing URL parsing. See [using-the-manifest.md](using-the-manifest.md). The **reference Cloudflare Worker** reads `MUX_MANIFEST_JSON` at runtime (after validating `version` and `routes`). On the Worker, `listen` is ignored today; only `routes` drive edge routing. A VPS-side multiplexer should honor `listen` as documented in [vps.md](vps.md).
